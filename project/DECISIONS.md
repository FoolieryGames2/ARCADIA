...
## D-0033 — Resident loopback transport and honest Recipe 0 base slice

- Date: 2026-09-04
- Status: accepted qualification infrastructure; Gates A2/A3 and Phase B remain open
- Decision: Add an independently hash-pinned `llama-server` target from the same llama.cpp commit and CUDA build, bind it only to `127.0.0.1`, disable its web UI and monitoring endpoints, and let one lab session own its process lifetime. Reuse resident model weights while issuing stateless requests without a conversation identifier or prompt cache. Preserve the original per-process direct transport as an explicit setting. Add a qualification-only BASE_ONLY invoker that performs AAE serialization, final `CALL_DATA` reparse, exact input-token counting, aggregate budget reservation, schema-constrained generation, strict output and mode-specific semantic validation, and activation receipt creation. Expose only the implemented zero-history Recipe 0 path and report Recipe 1 as `NOT_IMPLEMENTED`.
- Reason: Reloading the 2.5 GB model for every semantic call makes a multi-specialist pipeline impractical even though all layers are GPU-offloaded. A loopback resident server reduces warm calls from repeated cold-start latency to subsecond direct responses while preserving fresh request state. Stopping at the first absent controller prevents a schema catalog from being misrepresented as an executable Recipe 0–8 system and gives the next Recipe 1 implementation a real measured runtime boundary.

## D-0034 — Recipe-default operator routing and process-owned resident port
