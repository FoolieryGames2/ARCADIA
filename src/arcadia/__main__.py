...
def _print_runtime_checks(identity: RuntimeIdentity, *, full_model_hash: bool) -> int:
    checks = verify_runtime_files(identity, full_model_hash=full_model_hash)
    for check in checks:
...
def _interactive(workspace: Path, settings: LabSettings, *, metrics: bool) -> int:
    ...
    while True:
        ...
        if prompt == "/reset":
            ...
            continue
        if prompt == "/verify":
            _print_runtime_checks(identity, full_model_hash=True)
            continue
        ...
        if prompt.startswith("/"):
            ...
            continue
        try:
            response = run_base_prompt(identity, settings, prompt)
...
def _run_command(args: argparse.Namespace) -> int:
    ...
    if args.reset_settings:
        ...
        return 0
    if args.verify:
        return _print_runtime_checks(identity, full_model_hash=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (LabConfigError, LabRuntimeError) as exc:
